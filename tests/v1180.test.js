...
test('v1.32 incomplete first page cannot close chats; complete deep inventory is authoritative',()=>{
  assert.match(poller,/A full active-chat inventory is authoritative/);
  assert.match(poller,/await livechat\.listAllActiveChats\(\)/);
  assert.match(poller,/await db\.reconcileInboxVisibility\(seenChatIds,25\)/);
});
