...
test('getChat uses documented latest-thread request first and only falls back when it is empty',()=>{
  const s=fs.readFileSync(new URL('../src/livechat.js',import.meta.url),'utf8');
  assert.doesNotMatch(s,/thread_limit:\s*100/);
  assert.match(s,/candidates=\[\{chat_id:id\}\]/);
  assert.match(s,/if\(i===0 && count>0\) return chat/);
