...
test('active AI context is scoped by persisted session_key and welcome-only events cannot manufacture a session',()=>{
  ...
  assert.match(sessionIdentity,/providerThreadKey\?'LIVECHAT_THREAD_ID':null/);
  assert.match(engine,/if\(providerThreadKey\)\{/);
