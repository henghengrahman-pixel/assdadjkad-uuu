...
function renderRows(){
  ...
  if(!items.length){const msg=state.lane==='TRAFFIC'&&state.trafficStatus==='TRAFFIC_PROVIDER_DATA_UNAVAILABLE'?'TRAFFIC_PROVIDER_DATA_UNAVAILABLE — provider browsing presence tidak tersedia pada integrasi ini.':'Tidak ada percakapan sesuai filter.';list.innerHTML=`<div class="empty inbox-empty">${esc(msg)}</div>`;$('#inboxCount').textContent='0 aktif';$('#keyboardNavCount').textContent='0';return}
...
async function fetchList({append=false,silent=false,reason='user'}={}){
  ...
  try{
    ...
    state.hasMore=Boolean(data.page?.hasMore);state.cursor=data.page?.nextCursor||null;state.trafficStatus=data.traffic?.status||null;
