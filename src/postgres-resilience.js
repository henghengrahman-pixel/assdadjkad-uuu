...
export function attachClientErrorHandler(client,{name='postgres-client',logger=console}={}){
  if(!client || typeof client.on!=='function')return false;
  const marker=Symbol.for('livechat-ai.pg-client-error-handler');
  if(client[marker])return false;
  Object.defineProperty(client,marker,{value:true,enumerable:false,configurable:false});
  client.on('error',(error)=>{
    logger.error?.(JSON.stringify({timestamp:new Date().toISOString(),level:'error',module:'postgres',event:'CLIENT_CONNECTION_ERROR',client:name,...safePgError(error)}));
  });
  return true;
}
...
export function attachPoolErrorHandler(target,{name='postgres',logger=console}={}){
  ...
  target.on('connect',(client)=>attachClientErrorHandler(client,{name:`${name}:client`,logger}));
