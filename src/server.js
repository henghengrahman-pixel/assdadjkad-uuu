...
async function refreshDashboardLaneChat(chatId,{force=false}={}){
  ...
  const syncAt=Date.parse(st.last_livechat_sync_at||'');
  const stale=!Number.isFinite(syncAt)||(Date.now()-syncAt)>=config.lcDetailStaleMs;
  const blank=Number(st.message_count||0)===0;
  if(!force && lane==='MY_CHAT' && !blank && !stale) return {ok:true,lane,skipped:'fresh_my_chat'};
...
async function claimDashboardChat(chatId){
  ...
  const blocked=await isLiveChatClaimBlocked(id).catch(()=>({blocked:false}));
  if(blocked.blocked){const e=new Error(blocked.error||'LIVECHAT_CLAIM_COOLDOWN');e.code=blocked.error||'LIVECHAT_CLAIM_COOLDOWN';e.status=409;e.cooldownUntil=blocked.blockedUntil;throw e;}
  try{
    // Validate the provider state while holding the caller's per-chat lock. Do not claim a
    // stale/closed row and do not infer MY_CHAT from local state alone.
    const before=await lc.getChat(id,{id});
    const beforeState=lc.chatState(before); const beforeLane=lc.classifyChatLane(before);
    if(beforeState.active===false){const e=new Error('LIVECHAT_CHAT_INACTIVE');e.status=410;throw e;}
    if(beforeLane==='MY_CHAT'){await syncLaneChatForOperator(lc,id,before);await recordLiveChatClaimSuccess(id).catch(()=>{});return {ok:true,lane:'MY_CHAT',alreadyClaimed:true,providerLane:'MY_CHAT'};}
    const claimed=await lc.claimChat(id,{ensureMembership:true});
    const providerLane=String(claimed?.lane||lc.classifyChatLane(claimed?.chat||{})).toUpperCase();
    const refreshed=await syncLaneChatForOperator(lc,id,claimed.chat||{id});
    const confirmedLane=String(refreshed?.lane||providerLane||'OTHER').toUpperCase();
    if(providerLane!=='MY_CHAT' && confirmedLane!=='MY_CHAT'){
      await lc.unfollowChat?.(id).catch(()=>{});
      await recordLiveChatClaimFailure(id,{code:'LIVECHAT_CLAIM_OWNERSHIP_NOT_CONFIRMED',cooldownSeconds:config.lcClaimCooldownSeconds}).catch(()=>{});
      const e=new Error('LIVECHAT_CLAIM_OWNERSHIP_NOT_CONFIRMED');e.code='LIVECHAT_CLAIM_OWNERSHIP_NOT_CONFIRMED';e.status=409;throw e;
    }
    ...
    await recordLiveChatClaimSuccess(id).catch(()=>{});
    return {ok:true,lane:'MY_CHAT',providerLane:confirmedLane,membership:claimed.membership||null};
  }catch(e){
    const publicLimit=String(e?.code||'')==='LIVECHAT_PUBLIC_AGENT_LIMIT'||/PUBLIC_AGENT_LIMIT/i.test(String(e?.message||''));
    if(publicLimit){await recordLiveChatClaimFailure(id,{code:'LIVECHAT_PUBLIC_AGENT_LIMIT',cooldownSeconds:config.lcClaimCooldownSeconds}).catch(()=>{});e.retryable=false;e.status=409;}
    throw e;
  }
}
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{
  ...
  const [r,countRows]=await Promise.all([
    ...
    COALESCE(c.livechat_activity_at,c.last_event_at,c.updated_at) AS sort_time
    ...
    WHERE c.visible_in_inbox=true AND c.status='active' AND c.lc_active IS DISTINCT FROM false
      ...
      AND upper(trim(coalesce(c.lc_lane,'OTHER')))=$7
      AND ($5::timestamptz IS NULL OR COALESCE(c.lc_inbox_rank,2147483647)>COALESCE($4::int,2147483647) OR (COALESCE(c.lc_inbox_rank,2147483647)=COALESCE($4::int,2147483647) AND COALESCE(c.livechat_activity_at,c.last_event_at,c.updated_at)<$5::timestamptz) OR (COALESCE(c.lc_inbox_rank,2147483647)=COALESCE($4::int,2147483647) AND COALESCE(c.livechat_activity_at,c.last_event_at,c.updated_at)=$5::timestamptz AND c.chat_id>$6::text))
    ORDER BY COALESCE(c.lc_inbox_rank,2147483647) ASC, COALESCE(c.livechat_activity_at,c.last_event_at,c.updated_at) DESC, c.chat_id ASC
    LIMIT $3`,params),
  ...
  for(const row of countRows.rows||[]){const key=String(row.lc_lane||'OTHER').trim().toUpperCase();if(Object.prototype.hasOwnProperty.call(laneCounts,key) && key!=='TRAFFIC') laneCounts[key]=Number(row.count||0);}
  laneCounts.TRAFFIC=0;
  const traffic={providerDataAvailable:false,status:'TRAFFIC_PROVIDER_DATA_UNAVAILABLE',reason:'Provider browsing-presence data is not available through the configured LiveChat integration.'};
  res.json({ok:true,items:lane==='TRAFFIC'?[]:items,lane,laneCounts,traffic,page:{limit,nextCursor:lane==='TRAFFIC'?null:nextCursor,hasMore:lane==='TRAFFIC'?false:hasMore}});
});
app.get('/api/conversations/:id',requireAdmin,async(req,res)=>{
  ...
  if(pre && String(pre.status||'active').toLowerCase()!=='closed'){
...
app.get('/api/conversations/:id/messages',requireAdmin,async(req,res)=>{
  ...
  const pre=await getConversationState(req.params.id).catch(()=>null);
  if(pre && String(pre.status||'active').toLowerCase()!=='closed') await refreshDashboardLaneChat(req.params.id).catch(()=>{});
