...
export function topicFamily(intent='GENERAL'){
  ...
  if(x==='ACCOUNT_CHANGE_REQUEST'||x==='BANK_ACCOUNT_LIMIT'||x==='ACCOUNT_CLOSE_REQUEST') return 'ACCOUNT';
...
export function shouldHardSwitch({previousIntent='',freshIntent='',text=''}) {
  ...
  const explicit=/(deposit|depo|dpo|wd|withdraw|penarikan|password|sandi|login|akun|tutup|hapus|nonaktif|blokir|link|daftar|register|bonus|ronda|freebet|rollingan|rekening|bank|game|permainan|kemenangan|payout|jp|kalah|rungkad)/i.test(String(text||''));
