const typoMap = new Map(Object.entries({
  ...
  'wd':'withdraw','wede':'withdraw','withdrw':'withdraw','widraw':'withdraw','wdraw':'withdraw','witdrow':'withdraw','witdraw':'withdraw','widhraw':'withdraw','widhdraw':'withdraw','withdaw':'withdraw','withdrow':'withdraw','widrew':'withdraw','wdrew':'withdraw','wtdrw':'withdraw','penarikn':'withdraw','penarikanx':'withdraw','cashout':'withdraw','cairin':'withdraw',
  'dp':'deposit','dpo':'deposit','dps':'deposit','depo':'deposit','depoo':'deposit','depsit':'deposit','dposit':'deposit','depsoit':'deposit','depost':'deposit','depoist':'deposit','depposit':'deposit','depoit':'deposit','depsoitt':'deposit','topup':'deposit',
  'psw':'password','pwd':'password','lupapsw':'lupa password','pw':'password','pass':'password','passwd':'password','paswd':'password','paswrd':'password','passwrd':'password','paswod':'password','pasword':'password','passwordd':'password','sandi':'password','sndi':'password','sandiku':'password','sandy':'password','resset':'reset','resett':'reset',
  ...
  'gmn':'gimana','gmna':'gimana','knp':'kenapa','kpn':'kapan','skrg':'sekarang','tdk':'tidak','klw':'kalau','klo':'kalau','krn':'karena','karna':'karena','trs':'terus','trus':'terus','mksd':'maksud','mksdnya':'maksudnya','batalin':'batalkan','cancelin':'batalkan','brcd':'barcode','qrnya':'qr','qrisnya':'qris','slmt':'selamat','smt':'selamat','mlm':'malam','malem':'malam','pgi':'pagi','pg':'pagi','sre':'sore','sor':'sore','sng':'siang','gngguan':'gangguan','gangguanx':'gangguan','aksess':'akses','aksesx':'akses','limid':'limit','limitt':'limit','eror':'error','err':'error','errorr':'error','gagall':'gagal','rolingan':'rollingan','rolllingan':'rollingan','rolinggan':'rollingan','rolling':'rollingan','loginn':'login','ligin':'login','lgin':'login','lupa':'lupa','lpa':'lupa','lp':'lupa','lup':'lupa','alihin':'alihkan','alihkn':'alihkan','rek':'rekening',
...
export function detectIntent(text='') {
  ...
  // Account-close/access-disable language is NOT a forgot-password request.
  const accountCloseRequest =
    any('tutup akun','hapus akun','delete akun','nonaktifkan akun','non aktifkan akun','blokir akun','bekukan akun','matikan akun') ||
    (any('tidak mau login lagi','tidak bakal login lagi','tidak ingin login lagi','jangan bisa login lagi','biar tidak bisa login lagi') && any('akun','login','password')) ||
    (t.includes('ganti password') && any('biar tidak tahu','supaya tidak tahu','agar tidak tahu','tidak bakal login lagi','tidak mau login lagi'));
  if (accountCloseRequest) return 'ACCOUNT_CLOSE_REQUEST';
  ...
  if ((t.includes('withdraw') || t.includes('penarikan')) &&
      any('belum','pending','tidak masuk','belum masuk','belum cair','tidak cair','gangguan','masalah','proses','lama','gagal','status','cek','alihkan','antrian','tolong','bantu','selesaikan','diselesaikan','lanjutkan','percepat','dipercepat','kepastian','mana','gimana')) return 'WITHDRAW_PROBLEM';
  if ((t.includes('deposit')) &&
      any('belum','pending','tidak masuk','belum masuk','saldo belum','gangguan','masalah','proses','lama','gagal','status','cek','bukti','transfer','saldo tidak','tolong','bantu','selesaikan','diselesaikan','lanjutkan','percepat','dipercepat','kepastian','mana','gimana')) return 'DEPOSIT_PROBLEM';
