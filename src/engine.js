...
import { resolveGreetingSessionIdentity } from './session-identity.js';
...
export async function processGreetingTrigger({chatId,eventId,threadId=null,text,createdAt,livechat}){
  return db.withChatLock(chatId, async()=>{
    const {rawThreadKey,bannerEventKey,providerThreadKey,greetingKey}=resolveGreetingSessionIdentity({threadId,eventId,createdAt});
    ...
    // Greeting idempotency and session identity are intentionally separate. A welcome
    // event can justify a greeting, but it is NOT evidence of a new provider session.
    // Only a provider thread id can participate in session-boundary creation.
    ...
    if(!(await db.claimGreetingForThread(chatId,greetingKey))){
      return {skipped:'greeting_already_sent_for_session'};
    }

    if(providerThreadKey){
      await db.beginNewConversationSession(chatId,{triggerEventId:eventId,threadKey:providerThreadKey,boundarySource:'LIVECHAT_THREAD_ID'});
    }
      ...
      await db.logAI({
        ...
        reason:freshAuthoritativeBanner
          ...
          : rawThreadKey
            ...
            : 'LiveChat welcome greeting without provider session boundary'
      }).catch(()=>{});
